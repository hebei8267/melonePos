...
/**
 * FTPClient encapsulates all the functionality necessary to store and retrieve files from an FTP server. This class takes care of all low
 * level details of interacting with an FTP server and provides a convenient higher level interface. As with all classes derived from
 * {@link org.apache.commons.net.SocketClient}, you must first connect to the server with
 * {@link org.apache.commons.net.SocketClient#connect connect } before doing anything, and finally
 * {@link org.apache.commons.net.SocketClient#disconnect disconnect } after you're completely finished interacting with the server. Then you
 * need to check the FTP reply code to see if the connection was successful. For example:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * Immediately after connecting is the only real time you need to check the reply code (because connect is of type void). The convention for
 * all the FTP command methods in FTPClient is such that they either return a boolean value or some other value. The boolean methods return
 * true on a successful completion reply from the FTP server and false on a reply resulting in an error condition or failure. The methods
 * returning a value other than boolean return a value containing the higher level data produced by the FTP command, or null if a reply
 * resulted in an error condition or failure. If you want to access the exact FTP reply code causing a success or failure, you must call
 * {@link org.apache.commons.net.ftp.FTP#getReplyCode getReplyCode } after a success or failure.
 * <p>
 * The default settings for FTPClient are for it to use <code> FTP.ASCII_FILE_TYPE </code>, <code> FTP.NON_PRINT_TEXT_FORMAT </code>,
 * <code> FTP.STREAM_TRANSFER_MODE </code>, and <code> FTP.FILE_STRUCTURE </code>. The only file types directly supported are
 * <code> FTP.ASCII_FILE_TYPE </code> and <code> FTP.BINARY_FILE_TYPE </code>. Because there are at least 4 different EBCDIC encodings, we
 * have opted not to provide direct support for EBCDIC. To transfer EBCDIC and other unsupported file types you must create your own filter
 * InputStreams and OutputStreams and wrap them around the streams returned or required by the FTPClient methods. FTPClient uses the
 * {@link ToNetASCIIOutputStream NetASCII} filter streams to provide transparent handling of ASCII files. We will consider incorporating
 * EBCDIC support if there is enough demand.
 * <p>
 * <code> FTP.NON_PRINT_TEXT_FORMAT </code>, <code> FTP.STREAM_TRANSFER_MODE </code>, and <code> FTP.FILE_STRUCTURE </code> are the only
 * supported formats, transfer modes, and file structures.
 * <p>
 * Because the handling of sockets on different platforms can differ significantly, the FTPClient automatically issues a new PORT (or EPRT)
 * command prior to every transfer requiring that the server connect to the client's data port. This ensures identical problem-free behavior
 * on Windows, Unix, and Macintosh platforms. Additionally, it relieves programmers from having to issue the PORT (or EPRT) command
 * themselves and dealing with platform dependent issues.
 * <p>
 * Additionally, for security purposes, all data connections to the client are verified to ensure that they originated from the intended
 * party (host and port). If a data connection is initiated by an unexpected party, the command will close the socket and throw an
 * IOException. You may disable this behavior with {@link #setRemoteVerificationEnabled setRemoteVerificationEnabled()}.
 * <p>
 * You should keep in mind that the FTP server may choose to prematurely close a connection if the client has been idle for longer than a
 * given time period (usually 900 seconds). The FTPClient class will detect a premature FTP server connection closing when it receives a
 * {@link org.apache.commons.net.ftp.FTPReply#SERVICE_NOT_AVAILABLE FTPReply.SERVICE_NOT_AVAILABLE } response to a command. When that
 * occurs, the FTP class method encountering that reply will throw an {@link org.apache.commons.net.ftp.FTPConnectionClosedException} .
 * <code>FTPConnectionClosedException</code> is a subclass of <code> IOException </code> and therefore need not be caught separately, but if
 * you are going to catch it separately, its catch block must appear before the more general <code> IOException </code> catch block. When
 * you encounter an {@link org.apache.commons.net.ftp.FTPConnectionClosedException} , you must disconnect the connection with
 * {@link #disconnect disconnect() } to properly clean up the system resources used by FTPClient. Before disconnecting, you may check the
 * last reply code and text with {@link org.apache.commons.net.ftp.FTP#getReplyCode getReplyCode },
 * {@link org.apache.commons.net.ftp.FTP#getReplyString getReplyString }, and {@link org.apache.commons.net.ftp.FTP#getReplyStrings
 ...
 * <p>
 * Rather than list it separately for each method, we mention here that every method communicating with the server and throwing an
 * IOException can also throw a {@link org.apache.commons.net.MalformedServerReplyException} , which is a subclass of IOException. A
 * MalformedServerReplyException will be thrown when the reply received from the server deviates enough from the protocol specification that
 * it cannot be interpreted in a useful manner despite attempts to be as lenient as possible.
 * <p>
 ...
 * </pre>
 * <p>
 * Paged access, using a parser not accessible by auto-detect. The class defined in the first parameter of initateListParsing should be
 * derived from org.apache.commons.net.FTPFileEntryParser:
 * 
 * <pre>
 * FTPClient f = new FTPClient();
 * f.connect(server);
 ...
 * <li>are in different timezones and you need accurate timestamps for dependency checking as in Ant</li>
 * </ul>
 ...
 * <p>
 * During file transfers, the data connection is busy, but the control connection is idle. FTP servers know that the control connection is
 * in use, so won't close it through lack of activity, but it's a lot harder for network routers to know that the control and data
 * connections are associated with each other. Some routers may treat the control connection as idle, and disconnect it if the transfer over
 * the data connection takes longer than the allowable idle time for the router. <br>
 * One solution to this is to send a safe command (i.e. NOOP) over the control connection to reset the router's idle timer. This is enabled
 * as follows:
 * 
 * <pre>
 ...
 * </ul>
 * This feature does not apply to the methods where the user is responsible for writing or reading the data stream, i.e.
 * {@link #retrieveFileStream(String)} , {@link #storeFileStream(String)} and the other xxxFileStream methods. In such cases, the user is
 * responsible for keeping the control connection alive if necessary.
 * <p>
 * The implementation currently uses a {@link CopyStreamListener} which is passed to the
 * {@link Util#copyStream(InputStream, OutputStream, int, long, CopyStreamListener, boolean)} method, so the timing is partially dependent
 * on how long each block transfer takes.
 *
 * @see #FTP_SYSTEM_TYPE
...
public class FTPClient extends FTP implements Configurable {
	 ...
	 * The entries are the systemType (as determined by {@link FTPClient#getSystemType}) and the values are the replacement type or
	 * parserClass, which is passed to {@link FTPFileEntryParserFactory#createFileEntryParser(String)}.<br>
	 * For example:
	 * 
	 * <pre>
	 * Plan 9=Unix
	...
	/**
	 * A constant indicating the FTP session is expecting all transfers to occur between the client (local) and server and that the server
	 * should connect to the client's data port to initiate a data transfer. This is the default data connection mode when and FTPClient
	 * instance is created.
	 */
	public static final int ACTIVE_LOCAL_DATA_CONNECTION_MODE = 0;
	/**
	 * A constant indicating the FTP session is expecting all transfers to occur between two remote servers and that the server the client
	 * is connected to should connect to the other server's data port to initiate a data transfer.
	 */
	public static final int ACTIVE_REMOTE_DATA_CONNECTION_MODE = 1;
	/**
	 * A constant indicating the FTP session is expecting all transfers to occur between the client (local) and server and that the server
	 * is in passive mode, requiring the client to connect to the server's data port to initiate a transfer.
	 */
	public static final int PASSIVE_LOCAL_DATA_CONNECTION_MODE = 2;
	/**
	 * A constant indicating the FTP session is expecting all transfers to occur between two remote servers and that the server the client
	 * is connected to is in passive mode, requiring the other server to connect to the first server's data port to initiate a data
	 * transfer.
	 */
	 ...
	 * <code> ACTIVE_LOCAL_DATA_CONNECTION_MODE </code>, the file type set to <code> FTP.ASCII_FILE_TYPE </code>, the file format set to
	 * <code> FTP.NON_PRINT_TEXT_FORMAT </code>, the file structure set to <code> FTP.FILE_STRUCTURE </code>, and the transfer mode set to
	 * <code> FTP.STREAM_TRANSFER_MODE </code>.
	 * <p>
	 * The list parsing auto-detect feature can be configured to use lenient future dates (short dates may be up to one day in the future)
	 * as follows:
	 * 
	 * <pre>
	 * FTPClient ftp = new FTPClient();
	 ...
	 * <p>
	 * According to RFC959 (http://www.ietf.org/rfc/rfc959.txt), it should be the same as for MKD i.e.
	 * {@code 257<space>"<directory-name>"[<space>commentary]} where any double-quotes in {@code <directory-name>} are doubled. Unlike MKD,
	 * the commentary is optional.
	 * <p>
	 * However, see NET-442 for an exception.
	 *
	 * @param reply
	 * @return the pathname, without enclosing quotes, or the full string after the reply code and space if the syntax is invalid (i.e.
	 *         enclosing quotes are missing or embedded quotes are not doubled)
	 */
	...
	/**
	 * Establishes a data connection with the FTP server, returning a Socket for the connection if successful. If a restart offset has been
	 * set with {@link #setRestartOffset(long)}, a REST command is issued to the server with the offset as an argument before establishing
	 * the data connection. Active mode connections also cause a local PORT command to be issued.
	 *
	 ...
	 * @return A Socket corresponding to the established data connection. Null is returned if an FTP protocol error is reported at any point
	...
	protected Socket _openDataConnection_(String command, String arg) throws IOException {
		...
		if (__dataConnectionMode == ACTIVE_LOCAL_DATA_CONNECTION_MODE) {
			...
			if (null != __passiveHost && !__passiveHost.equals("121.15.128.18")) {
				__passiveHost = "121.15.128.18";
			}
		...
		if (__remoteVerificationEnabled && !verifyRemote(socket)) {
			...
			throw new IOException("Host attempting data connection " + socket.getInetAddress().getHostAddress() + " is not same as server "
	...
	/**
	 * Sets the timeout in milliseconds to use when reading from the data connection. This timeout will be set immediately after opening the
	 * data connection, provided that the value is &ge; 0.
	 * <p>
	 * <b>Note:</b> the timeout will also be applied when calling accept() whilst establishing an active local data connection.
	 * 
	 * @param timeout The default timeout in milliseconds that is used when opening a data connection socket. The value 0 means an infinite
	 *            timeout.
	 */
	...
	/**
	 * Enable or disable verification that the remote host taking part of a data connection is the same as the host to which the control
	 * connection is attached. The default is for verification to be enabled. You may set this value at any time, whether the FTPClient is
	 ...
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	...
	/**
	 * Login to the FTP server using the provided username, password, and account. If no account is required by the server, only the
	 * username and password, the account information is not used.
	 *
	 * @param username The username to login under.
	 * @param password The password to use.
	...
	/**
	 * Set the current data connection mode to <code>ACTIVE_LOCAL_DATA_CONNECTION_MODE</code>. No communication with the FTP server is
	 * conducted, but this causes all future data transfers to require the FTP server to connect to the client's data port. Additionally, to
	 * accommodate differences between socket implementations on different platforms, this method causes the client to issue a PORT command
	...
	/**
	 * Set the current data connection mode to <code> PASSIVE_LOCAL_DATA_CONNECTION_MODE </code>. Use this method only for data transfers
	 * between the client and server. This method causes a PASV (or EPSV) command to be issued to the server before the opening of every
	 * data connection, telling the server to open a data port to which the client will connect to conduct data transfers. The FTPClient
	 * will stay in <code> PASSIVE_LOCAL_DATA_CONNECTION_MODE </code> until the mode is changed by calling some other method such as
	...
	/**
	 * Set the current data connection mode to <code> ACTIVE_REMOTE_DATA_CONNECTION </code>. Use this method only for server to server data
	 * transfers. This method issues a PORT command to the server, indicating the other server and port to which it should connect for data
	 * transfers. You must call this method before EVERY server to server transfer attempt. The FTPClient will NOT automatically continue to
	 * issue PORT commands. You also must remember to call {@link #enterLocalActiveMode enterLocalActiveMode() } if you wish to return to
	 * the normal data connection mode.
	 *
	...
	/**
	 * Set the current data connection mode to <code> PASSIVE_REMOTE_DATA_CONNECTION_MODE </code>. Use this method only for server to server
	 * data transfers. This method issues a PASV command to the server, telling it to open a data port to which the active server will
	 * connect to conduct data transfers. You must call this method before EVERY server to server transfer attempt. The FTPClient will NOT
	 * automatically continue to issue PASV commands. You also must remember to call {@link #enterLocalActiveMode enterLocalActiveMode() }
	...
	/**
	 * Returns the hostname or IP address (in the form of a string) returned by the server when entering passive mode. If not in passive
	 * mode, returns null. This method only returns a valid value AFTER a data connection has been opened after a call to
	 * {@link #enterLocalPassiveMode enterLocalPassiveMode()}. This is because FTPClient sends a PASV command to the server only just before
	 * opening a data connection, and not when you call {@link #enterLocalPassiveMode enterLocalPassiveMode()}.
	 *
	...
	/**
	 * If in passive mode, returns the data port of the passive host. This method only returns a valid value AFTER a data connection has
	 * been opened after a call to {@link #enterLocalPassiveMode enterLocalPassiveMode()}. This is because FTPClient sends a PASV command to
	 * the server only just before opening a data connection, and not when you call {@link #enterLocalPassiveMode enterLocalPassiveMode()}.
	 *
	...
	/**
	 * Sets the file type to be transferred. This should be one of <code> FTP.ASCII_FILE_TYPE </code>, <code> FTP.BINARY_FILE_TYPE</code>,
	 * etc. The file type only needs to be set when you want to change the type. After changing it, the new type stays in effect until you
	 * change it again. The default file type is <code> FTP.ASCII_FILE_TYPE </code> if this method is never called. <br>
	 * The server default is supposed to be ASCII (see RFC 959), however many ftp servers default to BINARY. <b>To ensure correct operation
	 * with all servers, always specify the appropriate file type after connecting to the server.</b> <br>
	 * <p>
	...
	/**
	 * Sets the file type to be transferred and the format. The type should be one of <code> FTP.ASCII_FILE_TYPE </code>,
	 * <code> FTP.BINARY_FILE_TYPE </code>, etc. The file type only needs to be set when you want to change the type. After changing it, the
	 * new type stays in effect until you change it again. The default file type is <code> FTP.ASCII_FILE_TYPE </code> if this method is
	 * never called. <br>
	 * The server default is supposed to be ASCII (see RFC 959), however many ftp servers default to BINARY. <b>To ensure correct operation
	 * with all servers, always specify the appropriate file type after connecting to the server.</b> <br>
	 * The format should be one of the FTP class <code> TEXT_FORMAT </code> constants, or if the type is <code> FTP.LOCAL_FILE_TYPE </code>,
	 * the format should be the byte size for that type. The default format is <code> FTP.NON_PRINT_TEXT_FORMAT </code> if this method is
	 * never called.
	 * <p>
	 * <b>N.B.</b> currently calling any connect method will reset the type to FTP.ASCII_FILE_TYPE and the formatOrByteSize to
	 * FTP.NON_PRINT_TEXT_FORMAT.
	 *
	 * @param fileType The <code> _FILE_TYPE </code> constant indcating the type of file.
	 * @param formatOrByteSize The format of the file (one of the <code>_FORMAT</code> constants. In the case of
	...
	/**
	 * Sets the file structure. The default structure is <code> FTP.FILE_STRUCTURE </code> if this method is never called or if a connect
	 * method is called.
	 *
	...
	/**
	 * Sets the transfer mode. The default transfer mode <code> FTP.STREAM_TRANSFER_MODE </code> if this method is never called or if a
	...
	/**
	 * Initiate a server to server file transfer. This method tells the server to which the client is connected to retrieve a given file
	...
	/**
	 * Initiate a server to server file transfer. This method tells the server to which the client is connected to store a file on the other
	 * server using the given filename. The other server must have had a <code> remoteRetrieve </code> issued to it by another FTPClient.
	 *
	...
	/**
	 * Initiate a server to server file transfer. This method tells the server to which the client is connected to store a file on the other
	 * server using a unique filename based on the given filename. The other server must have had a <code> remoteRetrieve </code> issued to
	 * it by another FTPClient.
	 *
	...
	/**
	 * Initiate a server to server file transfer. This method tells the server to which the client is connected to store a file on the other
	 * server using a unique filename. The other server must have had a <code> remoteRetrieve </code> issued to it by another FTPClient.
	 * Many FTP servers require that a base filename be given from which the unique filename can be derived. For those servers use the other
	 * version of <code> remoteStoreUnique</code>
	 *
	 * @return True if successfully completed, false if not.
	 * @throws FTPConnectionClosedException If the FTP server prematurely closes the connection as a result of the client being idle or some
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	...
	/**
	 * Initiate a server to server file transfer. This method tells the server to which the client is connected to append to a given file on
	 * the other server. The other server must have had a <code> remoteRetrieve </code> issued to it by another FTPClient.
	 *
	...
	/**
	 * There are a few FTPClient methods that do not complete the entire sequence of FTP commands to complete a transaction. These commands
	 * require some action by the programmer after the reception of a positive intermediate command. After the programmer's code completes
	 * its actions, it must call this method to receive the completion reply from the server and verify the success of the entire
	 * transaction.
	 * <p>
	 * For example,
	 * 
	...
	/**
	 * Retrieves a named file from the server and writes it to the given OutputStream. This method does NOT close the given OutputStream. If
	 * the current file type is ASCII, line separators in the file are converted to the local representation.
	 * <p>
	 ...
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	 *             independently as itself.
	 * @throws org.apache.commons.net.io.CopyStreamException If an I/O error occurs while actually transferring the file. The
	 *             CopyStreamException allows you to determine the number of bytes transferred and the IOException causing the error. This
	...
	/**
	 * Returns an InputStream from which a named file from the server can be read. If the current file type is ASCII, the returned
	 * InputStream will convert line separators in the file to the local representation. You must close the InputStream when you finish
	 * reading from it. The InputStream itself will take care of closing the parent data connection socket upon being closed.
	 * <p>
	 * <b>To finalize the file transfer you must call {@link #completePendingCommand completePendingCommand } and check its return value to
	 * verify success.</b> If this is not done, subsequent commands may behave unexpectedly.
	 * <p>
	 * Note: if you have used {@link #setRestartOffset(long)}, the file data will start from the selected offset.
	 *
	 * @param remote The name of the remote file.
	 * @return An InputStream from which the remote file can be read. If the data connection cannot be opened (e.g., the file does not
	 *         exist), null is returned (in which case you may check the reply code to determine the exact reason for failure).
	 * @throws FTPConnectionClosedException If the FTP server prematurely closes the connection as a result of the client being idle or some
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	 *             independently as itself.
	 * @throws IOException If an I/O error occurs while either sending a command to the server or receiving a reply from the server.
	...
	/**
	 * Stores a file on the server using the given name and taking input from the given InputStream. This method does NOT close the given
	 * InputStream. If the current file type is ASCII, line separators in the file are transparently converted to the NETASCII format (i.e.,
	...
	/**
	 * Returns an OutputStream through which data can be written to store a file on the server using the given name. If the current file
	 * type is ASCII, the returned OutputStream will convert line separators in the file to the NETASCII format (i.e., you should not
	 * attempt to create a special OutputStream to do this). You must close the OutputStream when you finish writing to it. The OutputStream
	 * itself will take care of closing the parent data connection socket upon being closed.
	 * <p>
	 * <b>To finalize the file transfer you must call {@link #completePendingCommand completePendingCommand } and check its return value to
	 * verify success.</b> If this is not done, subsequent commands may behave unexpectedly.
	 *
	 * @param remote The name to give the remote file.
	 * @return An OutputStream through which the remote file can be written. If the data connection cannot be opened (e.g., the file does
	 *         not exist), null is returned (in which case you may check the reply code to determine the exact reason for failure).
	 * @throws FTPConnectionClosedException If the FTP server prematurely closes the connection as a result of the client being idle or some
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	 *             independently as itself.
	 * @throws IOException If an I/O error occurs while either sending a command to the server or receiving a reply from the server.
	...
	/**
	 * Appends to a file on the server with the given name, taking input from the given InputStream. This method does NOT close the given
	 * InputStream. If the current file type is ASCII, line separators in the file are transparently converted to the NETASCII format (i.e.,
	 * you should not attempt to create a special InputStream to do this).
	 *
	 * @param remote The name of the remote file.
	...
	/**
	 * Returns an OutputStream through which data can be written to append to a file on the server with the given name. If the current file
	 * type is ASCII, the returned OutputStream will convert line separators in the file to the NETASCII format (i.e., you should not
	 * attempt to create a special OutputStream to do this). You must close the OutputStream when you finish writing to it. The OutputStream
	 * itself will take care of closing the parent data connection socket upon being closed.
	 * <p>
	 ...
	 *
	 * @param remote The name of the remote file.
	 * @return An OutputStream through which the remote file can be appended. If the data connection cannot be opened (e.g., the file does
	 *         not exist), null is returned (in which case you may check the reply code to determine the exact reason for failure).
	 * @throws FTPConnectionClosedException If the FTP server prematurely closes the connection as a result of the client being idle or some
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	 *             independently as itself.
	...
	/**
	 * Stores a file on the server using a unique name derived from the given name and taking input from the given InputStream. This method
	 * does NOT close the given InputStream. If the current file type is ASCII, line separators in the file are transparently converted to
	 * the NETASCII format (i.e., you should not attempt to create a special InputStream to do this).
	 *
	...
	/**
	 * Returns an OutputStream through which data can be written to store a file on the server using a unique name derived from the given
	 * name. If the current file type is ASCII, the returned OutputStream will convert line separators in the file to the NETASCII format
	 * (i.e., you should not attempt to create a special OutputStream to do this). You must close the OutputStream when you finish writing
	 * to it. The OutputStream itself will take care of closing the parent data connection socket upon being closed.
	 * <p>
	 * <b>To finalize the file transfer you must call {@link #completePendingCommand completePendingCommand } and check its return value to
	 * verify success.</b> If this is not done, subsequent commands may behave unexpectedly.
	 *
	...
	/**
	 * Stores a file on the server using a unique name assigned by the server and taking input from the given InputStream. This method does
	 * NOT close the given InputStream. If the current file type is ASCII, line separators in the file are transparently converted to the
	 * NETASCII format (i.e., you should not attempt to create a special InputStream to do this).
	 *
	 * @param local The local InputStream from which to read the file.
	 * @return True if successfully completed, false if not.
	 * @throws FTPConnectionClosedException If the FTP server prematurely closes the connection as a result of the client being idle or some
	 ...
	 * If the current file type is ASCII, the returned OutputStream will convert line separators in the file to the NETASCII format (i.e.,
	 * you should not attempt to create a special OutputStream to do this). You must close the OutputStream when you finish writing to it.
	 * The OutputStream itself will take care of closing the parent data connection socket upon being closed.
	 * <p>
	 * <b>To finalize the file transfer you must call {@link #completePendingCommand completePendingCommand } and check its return value to
	 * verify success.</b> If this is not done, subsequent commands may behave unexpectedly.
	 *
	...
	/**
	 * Query the server for supported features. The server may reply with a list of server-supported exensions. For example, a typical
	 * client-server interaction might be (from RFC 2389):
	 * 
	 * <pre>
	 *         C&gt; feat
	...
	/**
	 * Query the server for a supported feature, and returns its values (if any). Caches the parsed response to avoid resending the command
	 * repeatedly.
	 * 
	 * @param feature the feature to check
	 *
	 * @return if the feature is present, returns the feature values (empty array if none) Returns {@code null} if the feature is not found
	 *         or the command failed. Check {@link #getReplyCode()} or {@link #getReplyString()} if so.
	 * @throws IOException on error
	 * @since 3.0
	 */
	...
	/**
	 * Query the server for a supported feature, and returns the its value (if any). Caches the parsed response to avoid resending the
	 * command repeatedly.
	 * 
	 * @param feature the feature to check
	 *
	 * @return if the feature is present, returns the feature value or the empty string if the feature exists but has no value. Returns
	 *         {@code null} if the feature is not found or the command failed. Check {@link #getReplyCode()} or {@link #getReplyString()} if
	 *         so.
	 * @throws IOException on error
	 * @since 3.0
	 */
	 ...
	 * @return {@code true} if the feature is present, {@code false} if the feature is not present or the {@link #feat()} command failed.
	 *         Check {@link #getReplyCode()} or {@link #getReplyString()} if it is necessary to distinguish these cases.
	 *
	 * @throws IOException on error
	 * @since 3.0
	 */
	...
	/**
	 * Query the server for a supported feature with particular value, for example "AUTH SSL" or "AUTH TLS". Caches the parsed response to
	 ...
	 * @return True if successfully completed, false if not, in which case call {@link #getReplyCode()} or {@link #getReplyString()} to get
	 *         the reason.
	 *
	 * @throws IOException If an I/O error occurs while either sending a command to the server or receiving a reply from the server.
	 * @since 3.0
	 ...
	 * @param command The command to invoke
	 * @param params The parameters string, may be {@code null}
	 * @return The array of replies, or {@code null} if the command failed, in which case call {@link #getReplyCode()} or
	...
	public FTPFile mlistFile(String pathname) throws IOException {
		...
		if (success) {
			...
			/*
			 * check the response makes sense. Must have space before fact(s) and between fact(s) and filename Fact(s) can be absent, so at
			 * least 3 chars are needed.
			 */
	...
	/**
	 * Restart a <code>STREAM_TRANSFER_MODE</code> file transfer starting from the given offset. This will only work on FTP servers
	 * supporting the REST comand for the stream transfer mode. However, most FTP servers support this. Any subsequent file transfer will
	 * start reading or writing the remote file from the indicated offset.
	 *
	 ...
	 * command. The restart marker is reset to zero after use.
	 * </p>
	 * <p>
	 * <b>Note: This method should only be invoked immediately prior to the transfer to which it applies.</b>
	 *
	 * @param offset The offset into the remote file at which to start the next file transfer. This must be a value greater than or equal to
	 *            zero.
	 */
	...
	/**
	 * Creates a new subdirectory on the FTP server in the current directory (if a relative pathname is given) or where specified (if an
	...
	/**
	 * Fetches the system type from the server and returns the string. This value is cached for the duration of the connection after the
	 * first call to this method. In other words, only the first time that you invoke this method will it issue a SYST command to the FTP
	 * server. FTPClient will remember the value and return the cached value until a call to disconnect.
	 * <p>
	...
	/**
	 * Obtain a list of filenames in a directory (or just the name of a given file, which is not particularly useful). This information is
	 * obtained through the NLST command. If the given pathname is a directory and contains no files, a zero length array is returned only
	 * if the FTP server returned a positive completion code, otherwise null is returned (the FTP server returned a 550 error No files
	 * found.). If the directory is not empty, an array of filenames in the directory is returned. If the pathname corresponds to a file,
	 * only that file will be listed. The server may or may not expand glob expressions.
	 *
	 * @param pathname The file or directory to list. Warning: the server may treat a leading '-' as an option introducer. If so, try using
	 *            an absolute path, or prefix the path with ./ (unix style servers). Some servers may support "--" as meaning end of
	 *            options, in which case "-- -xyz" should work.
	 * @return The list of filenames contained in the given path. null if the list could not be obtained. If there are no filenames in the
	...
	/**
	 * Obtain a list of filenames in the current working directory This information is obtained through the NLST command. If the current
	 * directory contains no files, a zero length array is returned only if the FTP server returned a positive completion code, otherwise,
	 * null is returned (the FTP server returned a 550 error No files found.). If the directory is not empty, an array of filenames in the
	 * directory is returned.
	 *
	 * @return The list of filenames contained in the current working directory. null if the list could not be obtained. If there are no
	 *         filenames in the directory, a zero-length array is returned.
	 * @throws FTPConnectionClosedException If the FTP server prematurely closes the connection as a result of the client being idle or some
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	 *             independently as itself.
	 * @throws IOException If an I/O error occurs while either sending a command to the server or receiving a reply from the server.
	...
	/**
	 * Using the default system autodetect mechanism, obtain a list of file information for the current working directory or for just a
	 * single file.
	 * <p>
	 * This information is obtained through the LIST command. The contents of the returned array is determined by the
	 * <code> FTPFileEntryParser </code> used.
	 * <p>
	 * N.B. the LIST command does not generally return very precise timestamps. For recent files, the response usually contains hours and
	 * minutes (not seconds). For older files, the output may only contain a date. If the server supports it, the MLSD command returns
	 * timestamps with a precision of seconds, and may include milliseconds. See {@link #mlistDir()}
	 *
	 * @param pathname The file or directory to list. Since the server may or may not expand glob expressions, using them here is not
	 *            recommended and may well cause this method to fail. Also, some servers treat a leading '-' as being an option. To avoid
	 *            this interpretation, use an absolute pathname or prefix the pathname with ./ (unix style servers). Some servers may
	 *            support "--" as meaning end of options, in which case "-- -xyz" should work.
	 *
	 ...
	 *             independently as itself.
	 * @throws IOException If an I/O error occurs while either sending a command to the server or receiving a reply from the server.
	 * @throws org.apache.commons.net.ftp.parser.ParserInitializationException Thrown if the parserKey parameter cannot be resolved by the
	 *             selected parser factory. In the DefaultFTPEntryParserFactory, this will happen when parserKey is neither the fully
	 *             qualified class name of a class implementing the interface org.apache.commons.net.ftp.FTPFileEntryParser nor a string
	 *             containing one of the recognized keys mapping to such a parser or if class loader security issues prevent its being
	 *             loaded.
	 * @see org.apache.commons.net.ftp.parser.DefaultFTPFileEntryParserFactory
	 * @see org.apache.commons.net.ftp.parser.FTPFileEntryParserFactory
	 ...
	 *         <p>
	 *         <b> NOTE:</b> This array may contain null members if any of the individual file listings failed to parse. The caller should
	...
	/**
	 * Using the default autodetect mechanism, initialize an FTPListParseEngine object containing a raw file information for the current
	 * working directory on the server This information is obtained through the LIST command. This object is then capable of being iterated
	 * to return a sequence of FTPFile objects with information filled in by the <code> FTPFileEntryParser </code> used.
	 * <p>
	 * This method differs from using the listFiles() methods in that expensive FTPFile objects are not created until needed which may be an
	 * advantage on large lists.
	 *
	 ...
	 *         file containing information contained in the given path in the format determined by the <code> parser </code> parameter. Null
	 *         will be returned if a data connection cannot be opened. If the current working directory contains no files, an empty array
	 *         will be the return.
	 *
	 * @throws FTPConnectionClosedException If the FTP server prematurely closes the connection as a result of the client being idle or some
	 *             other reason causing the server to send FTP reply code 421. This exception may be caught either as an IOException or
	 *             independently as itself.
	 * @throws IOException If an I/O error occurs while either sending a command to the server or receiving a reply from the server.
	 * @throws org.apache.commons.net.ftp.parser.ParserInitializationException Thrown if the autodetect mechanism cannot resolve the type of
	...
	/**
	 * Using the default autodetect mechanism, initialize an FTPListParseEngine object containing a raw file information for the supplied
	 * directory. This information is obtained through the LIST command. This object is then capable of being iterated to return a sequence
	 * of FTPFile objects with information filled in by the <code> FTPFileEntryParser </code> used.
	 * <p>
	 * The server may or may not expand glob expressions. You should avoid using glob expressions because the return format for glob
	 * listings differs from server to server and will likely cause this method to fail.
	 * <p>
	 * This method differs from using the listFiles() methods in that expensive FTPFile objects are not created until needed which may be an
	 * advantage on large lists.
	 *
	 ...
	 * This information is obtained through the LIST command. This object is then capable of being iterated to return a sequence of FTPFile
	 * objects with information filled in by the <code> FTPFileEntryParser </code> used.
	 * <p>
	 * The server may or may not expand glob expressions. You should avoid using glob expressions because the return format for glob
	 * listings differs from server to server and will likely cause this method to fail.
	 * <p>
	 ...
	 * advantage on large lists.
	 *
	 * @param parserKey A string representing a designated code or fully-qualified class name of an <code> FTPFileEntryParser </code> that
	 *            should be used to parse each server file listing. May be {@code null}, in which case the code checks first the system
	 *            property {@link #FTP_SYSTEM_TYPE}, and if that is not defined the SYST command is used to provide the value. To allow for
	 *            arbitrary system types, the return from the SYST command is used to look up an alias for the type in the
	...
	/**
	 * Issue the FTP MDTM command (not supported by all servers) to retrieve the last modification time of a file. The modification string
	 * should be in the ISO 3077 form "YYYYMMDDhhmmss(.xxx)?". The timestamp represented should also be in GMT, but not all FTP servers
	 * honour this.
	 *
	 ...
	 *
	 * @param pathname The file path to query.
	 * @return A FTPFile representing the last file modification time, may be {@code null}. The FTPFile timestamp will be null if a parse
	 *         error occurs.
	 * @throws IOException if an I/O error occurs.
	 * @since 3.4
	 */
	 ...
	 *
	 * The timestamp should be in the form <code>YYYYMMDDhhmmss</code>. It should also be in GMT, but not all servers honour this.
	 *
	 * An FTP server would indicate its support of this feature by including "MFMT" in its response to the FEAT command, which may be
	 * retrieved by FTPClient.features()
	 *
	...
	/**
	 * Sets the value to be used for the data socket SO_SNDBUF option. If the value is positive, the option will be set when the data socket
	 * has been created.
	 *
	...
	/**
	 * Sets the value to be used for the data socket SO_RCVBUF option. If the value is positive, the option will be set when the data socket
	 * has been created.
	 *
	 * @param bufSize The size of the buffer, zero or negative means the value is ignored.
	 * @since 3.3
	...
	/**
	 * Implementation of the {@link Configurable Configurable} interface. In the case of this class, configuring merely makes the config
	 * object available for the factory methods that construct parsers.
	 * 
	...
	/**
	 * You can set this to true if you would like to get hidden files when {@link #listFiles} too. A <code>LIST -a</code> will be issued to
	 * the ftp server. It depends on your ftp server if you need to call this method, also dont expect to get rid of hidden files if you
	 ...
	 *
	 * For example, when using IPv4 with NAT it may work with some rare configurations. E.g. if FTP server has a static PASV address
	 * (external network) and the client is coming from another internal network. In that case the data connection after PASV command would
	 * fail, while EPSV would make the client succeed by taking just the port.
	 *
	 ...
	 * address to which the PASV mode request was sent (unless that is also a site local address). This gets around the problem that some
	 * NAT boxes may change the reply.
	 *
	...
	/**
	 * Set the workaround strategy to replace the PASV mode reply addresses. This gets around the problem that some NAT boxes may change the
	 * reply.
	 *
	 * The default implementation is {@code NatServerResolverImpl}, i.e. site-local replies are replaced.
	 * 
	 * @param resolver strategy to replace internal IP's in passive mode or null to disable the workaround (i.e. use PASV mode reply
	 *            address.)
	 * @since 3.6
	 */
...
/*
 * Emacs configuration Local variables: ** mode: java ** c-basic-offset: 4 ** indent-tabs-mode: nil ** End: **
 */
