...
<html>
	...
	<body>
			...
			<form method="post"	class="form-inline" id="listForm">
				<div class="row">
					...
					<div class="span8">
						...
						<input id="optDate_start" name="optDate_start" type="text" class="input-medium" value="${optDate_start }"/>
						～
						<input id="optDate_end" name="optDate_end" type="text" class="input-medium" value="${optDate_end }"/>
					</div>
		...
		<script>
			$(function() {
				$('#optDate_start').datepicker({
					format : 'yyyy-mm-dd'
				});
				$('#optDate_end').datepicker({
					format : 'yyyy-mm-dd'
				});
			...
			function initDateMode() {
				...
				if (dateMode == 'week') {//周对比
					$('#optDate_end').val(moment().add('days', -1).format("YYYY-MM-DD"));
					$('#optDate_start').val(moment().add('days', -7).format("YYYY-MM-DD"));
				} else {//月对比
					$('#optDate_end').val(moment().add('days', -1).format("YYYY-MM-DD"));
					$('#optDate_start').val(moment().add('months', -1).format("YYYY-MM-DD"));
				}
			}
		</script>
	</body>
